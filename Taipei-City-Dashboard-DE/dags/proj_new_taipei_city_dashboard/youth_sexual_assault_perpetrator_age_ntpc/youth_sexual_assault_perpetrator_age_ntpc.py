...
API = (
    ...
    "e871f626-2e65-420d-a49b-a83d3bccbe5e/json"
)
DATASET_PAGE = "https://data.ntpc.gov.tw/datasets/e871f626-2e65-420d-a49b-a83d3bccbe5e"
PAGE_SIZE = 5000
UA = (
...
AGE_BANDS = [
    ("0_5", 0, 5, "0-5歲", "percent6", "percent7"),
    ("6_11", 6, 11, "6-11歲", "percent8", "percent9"),
    ("12_17", 12, 17, "12-17歲", "percent10", "percent11"),
    ("18_23", 18, 23, "18-23歲", "percent12", "percent13"),
    ("24_29", 24, 29, "24-29歲", "percent14", "percent15"),
    ("30_39", 30, 39, "30-39歲", "percent16", "percent17"),
    ("40_49", 40, 49, "40-49歲", "percent18", "percent19"),
    ("50_64", 50, 64, "50-64歲", "percent20", "percent21"),
    ("65_plus", 65, None, "65歲以上", "percent22", "percent23"),
    ("unknown", None, None, "年齡不詳", "percent24", "percent25"),
]

COLUMN_ALIASES = {
    "year": ("field1",),
    "male_total": ("percent4", "percent 4"),
    "female_total": ("percent5", "percent 5"),
    "unknown_gender": ("percent26", "percent 26"),
}
...
def _text(value, field):
    ...
    if not value or value.lower() in {"nan", "none", "null"}:
        raise ValueError(f"性侵害加害者來源欄位 {field} 為空")
    return value
...
def _number(value, field):
    ...
    try:
        ...
        raise ValueError(f"性侵害加害者來源欄位 {field} 無法轉數值：{value!r}") from exc
    if number < 0 or number != number:
        raise ValueError(f"性侵害加害者來源欄位 {field} 出現無效數值：{value!r}")
    return number
...
def _normalise_record(record):
    ...
    for canonical, aliases in COLUMN_ALIASES.items():
        ...
        if not matches:
            raise ValueError(
                f"性侵害加害者來源缺少角色欄位 {canonical}；實際欄位：{list(record)}"
            )
        if len(matches) > 1 and record[matches[0]] != record[matches[1]]:
            raise ValueError(f"性侵害加害者來源欄位別名衝突：{matches}")
    ...
    with requests.Session() as session:
        while page < 100:
            ...
            if not isinstance(page_records, list):
                raise RuntimeError(f"性侵害加害者來源第 {page} 頁不是 JSON 陣列")
            print(f"sexual perpetrator page {page}: {len(page_records)} rows")
            if not page_records:
                return records
            records.extend(page_records)
            page += 1
    raise RuntimeError("性侵害加害者來源分頁超過 100 頁，疑似 API 行為改變。")
...
def _year_totals(records):
    ...
    for record in records:
        ...
        if year in totals:
            raise ValueError(f"性侵害加害者來源年度重複：{year}")
...
def drop_scope_anomalies(records):
    ...
    if len(totals) < 3:
        raise RuntimeError("性侵害加害者來源年度少於三年，無法做尺度檢核。")
    for year, value in sorted(totals.items()):
        ...
        if median and (value > median * SCALE_ANOMALY_FACTOR or value * SCALE_ANOMALY_FACTOR < median):
            raise ValueError(
                f"西元 {year} 年性侵害加害者 total={value:.0f}，"
                f"與其餘年度中位數 {median:.0f} 相差超過 {SCALE_ANOMALY_FACTOR} 倍。"
            )
    print(f"sexual perpetrator scale guard: {len(totals)} years passed")
    return records
...
def _reconcile(records, data):
    ...
    for row in normalised:
        year = int(_text(row["year"], "year"))
        for canonical in ("male_total", "female_total", "unknown_gender"):
    ...
    if set(expected) != set(emitted):
        raise ValueError("性侵害加害者來源對帳失敗：輸入欄位與輸出欄位集合不一致。")
    for key, value in expected.items():
        if abs(value - emitted[key]) > 0.5:
            raise ValueError(f"性侵害加害者來源對帳失敗：{key} input={value} output={emitted[key]}")
    ...
    for row in normalised:
        ...
        if abs(male_detail - male_total) > 0.5 or abs(female_detail - female_total) > 0.5:
            raise ValueError(
                f"性侵害加害者來源年度對帳失敗：{row['year']} "
                f"male {male_total}/{male_detail}, female {female_total}/{female_detail}"
            )
    print(f"sexual perpetrator reconciliation: {len(expected)} source cells preserved")
...
def transform_records(records, data_time=None):
    ...
    if not records:
        raise RuntimeError("性侵害加害者來源沒有任何資料列。")
    rows = []
    for record in records:
        row = _normalise_record(record)
        year = int(_text(row["year"], "year"))
        ...
        for gender, canonical in (("male", "male_total"), ("female", "female_total")):
            rows.append(
                dict(
                    base,
                    indicator_id="sexual_assault_perpetrator_total_count",
                    age_lower=None,
                    age_upper=None,
                    age_band_raw=None,
                    gender=gender,
        ...
        rows.append(
            dict(
                base,
                indicator_id="sexual_assault_perpetrator_unknown_gender_count",
                age_lower=None,
                age_upper=None,
                age_band_raw=None,
                gender="unknown",
                breakdown=json.dumps(
                    {"scope": "all_age", "source_column": "unknown_gender"},
                    ensure_ascii=False,
                ),
                value=_number(row["unknown_gender"], "unknown_gender"),
                data_time=data_time,
            )
        )
        for key, _, _, label, _, _ in AGE_BANDS:
            ...
            for gender in ("male", "female"):
                ...
                rows.append(
                    dict(
                        base,
                        indicator_id="sexual_assault_perpetrator_age_count",
                        age_lower=age_lower,
                        age_upper=age_upper,
                        age_band_raw=label,
                        gender=gender,
...
dag = CommonDag(
    ...
    dag_folder="youth_sexual_assault_perpetrator_age_ntpc",
)

...
KNOWN_BAD_YEARS = {}
INDICATOR_ID = "naturalized_settlement_population_count"
CONTRACT_COLUMNS = [
...
def _number(value, field):
    ...
    if not raw or raw.lower() in {"nan", "none", "null"}:
        raise ValueError(f"ODRP072 欄位 {field} 為空")
    try:
        number = float(raw.replace(",", ""))
    except ValueError as exc:
        raise ValueError(f"ODRP072 欄位 {field} 無法轉數值：{value!r}") from exc
    if not math.isfinite(number) or number < 0:
        raise ValueError(f"ODRP072 欄位 {field} 出現無效數值：{value!r}")
    return number
...
def _parse_age(label):
    ...
    if match:
        ...
        if age > 100:
            raise ValueError(f"ODRP072 年齡超出已知範圍：{label!r}")
        return age, age
    ...
    if match:
        ...
        if upper < 0:
            raise ValueError(f"ODRP072 年齡區間無效：{label!r}")
        return None, upper

    raise ValueError(f"無法解析 ODRP072 年齡標籤：{label!r}")
...
def _gender(value):
    try:
        ...
        raise ValueError(f"ODRP072 出現未知性別：{value!r}") from exc
...
def _area_code(record):
    ...
    if raw.startswith("65000"):
        raise ValueError(f"ODRP072 出現未列入契約的新北行政區代碼：{raw!r}")
    return None
...
def _fetch_page(session, period, page):
    ...
    if code != "OD-0101-S":
        raise RuntimeError(
            f"ODRP072 {period} PAGE={page} 回應碼異常："
            f"{code} {body.get('responseMessage')}"
        )
    records = body.get("responseData")
    if not isinstance(records, list):
        raise TypeError(f"ODRP072 {period} PAGE={page} responseData 不是陣列")
    try:
        total_pages = int(body.get("totalPage", 1))
    except (TypeError, ValueError) as exc:
        raise TypeError(f"ODRP072 {period} totalPage 無法解析") from exc
    return body, total_pages


def discover_periods():
    """從錨點年度向前後探測，連續兩個無資料年度才停止。"""
    import requests

    found = {}
    with requests.Session() as session:
        if _fetch_page(session, str(ANCHOR_ROC_YEAR), 1) is None:
            raise RuntimeError(f"ODRP072 錨點民國 {ANCHOR_ROC_YEAR} 年沒有資料")
        found[ANCHOR_ROC_YEAR] = str(ANCHOR_ROC_YEAR)
        ...
        for direction in (1, -1):
            ...
            while misses < 2:
                period = str(roc_year)
                if _fetch_page(session, period, 1) is None:
                    misses += 1
                else:
    ...
    print(f"ODRP072 discovered periods: {periods}")
    return periods
...
def fetch_period_aggregates(period):
    ...
    def consume(fetched, page_number):
        ...
        if fetched is None:
            raise RuntimeError(f"ODRP072 {period} 中途回傳查無資料：PAGE={page_number}")
        body, _ = fetched
        ...
        for record in records:
            ...
            if area_code is None:
                continue
            required = [
                "district_code", "site_id", "village", "origin",
                "sex", "age", "population",
            ]
            missing = [field for field in required if field not in record]
            if missing:
                raise ValueError(
                    f"ODRP072 {period} 新北資料缺少欄位 {missing}：{list(record)}"
                )
            origin = str(record["origin"]).strip()
            if origin not in ORIGIN_TYPES:
                raise ValueError(f"ODRP072 {period} 出現未知來源類別：{origin!r}")
            age_lower, age_upper = _parse_age(record["age"])
            gender = _gender(record["sex"])
            value = _number(record["population"], "population")
            key = (
                area_code,
                age_lower,
                age_upper,
    ...
    with requests.Session() as session:
        ...
        consume(first, 1)
        print(
            f"ODRP072 {period} page 1/{total_pages}: "
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            for start in range(2, total_pages + 1, workers):
                ...
                if end == total_pages or end % 100 < workers:
                    print(
                        f"ODRP072 {period} page {start}-{end}/{total_pages}: "
                        f"national={national_rows:,}, ntpc={ntpc_rows:,}"
                    )

    if not aggregates:
        raise RuntimeError(f"ODRP072 {period} 沒有新北市資料")
    return {
        "period": period,
        "roc_year": int(period),
...
def drop_scope_anomalies(period_results):
    ...
    if not totals:
        raise RuntimeError("ODRP072 沒有可供尺度檢核的年度")
    ...
    for year, value in sorted(active.items()):
        ...
        if median and (value > median * SCALE_ANOMALY_FACTOR
            ...
            raise ValueError(
                f"ODRP072 民國 {year} 年新住民來源人口 {value:,.0f}，"
    ...
    for year, reason in sorted(KNOWN_BAD_YEARS.items()):
        if year in totals:
            print(f"ODRP072 scale guard: 排除民國 {year}（{reason}）")
...
def build_ready_data(period_results, data_time=None):
    ...
    for result in period_results:
        ...
        district = period_data[
            (period_data["period_start"] == period_start)
            & (period_data["area_level"] == "district")
        ]
    ...
    if not rows:
        raise RuntimeError("ODRP072 轉換後沒有資料列")
    ...
    if data.empty:
        raise RuntimeError("ODRP072 濾除零值後沒有資料列")
...
dag = CommonDag(
    ...
    dag_folder="youth_naturalized_population_age_ntpc",
)

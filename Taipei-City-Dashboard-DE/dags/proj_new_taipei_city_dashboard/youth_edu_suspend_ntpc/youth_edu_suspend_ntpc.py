...
CURRENT_FILENAME = (
    "學13-2.於學年底處於休學狀態之人數-以「校(含學制班別)」統計(111學年度起).csv"
)
LEGACY_FILENAME = (
    "學13-2.於學年底處於休學狀態之人數-以「校(含學制班別)」統計(110學年度以前).csv"
)
CONTRACT_COLUMNS = [
...
INDICATOR_ID = "college_suspend_count"
...
def _resolve_columns(columns):
    ...
    roles = {
        ...
        "total": _find_first(columns, "休學狀態之人數", "總計"),
    ...
    if missing:
        raise ValueError(
            f"休學來源缺少必要欄位角色 {missing}；實際欄位：{columns}"
        )

    if roles["student_initiated_total"] and roles["school_ordered_total"]:
        if roles["total"] is None:
            raise ValueError(f"休學新格式缺少休學總計欄：{columns}")
        roles["mode"] = "split"
        roles["student_initiated_reasons"] = _find_all(
            columns, "學生自請休學原因", exclude=("小計",)
        )
        roles["school_ordered_reasons"] = _find_all(
            columns, "學校勒令休學原因", exclude=("小計",)
        )
        if not roles["student_initiated_reasons"] or not roles["school_ordered_reasons"]:
            raise ValueError(f"休學新格式找不到自請／勒令原因欄：{columns}")
    else:
        roles["mode"] = "legacy"
        if roles["total"] is None:
            roles["total"] = (
                _find_first(columns, "休學狀態之人數", "總計")
                or _find_first(columns, "休學狀態之人數", "小計")
            )
        if roles["total"] is None:
            raise ValueError(f"休學歷史格式缺少總計欄：{columns}")
        roles["legacy_reasons"] = [
            column for column in columns
            if "休學狀態之人數" in str(column)
            and column != roles["total"]
        ]
        if not roles["legacy_reasons"]:
            raise ValueError(f"休學歷史格式找不到原因欄：{columns}")
    return roles
...
def _gender(value):
    ...
    if text not in aliases:
        raise ValueError(f"休學來源出現未知性別值：{value!r}")
    return aliases[text]
...
def _parse_number(value, field, stats, kind="count"):
    ...
    try:
        ...
        raise ValueError(f"休學來源欄位 {field} 無法轉數值：{value!r}") from exc
    if not math.isfinite(number) or number < 0:
        raise ValueError(f"休學來源欄位 {field} 出現無效數值：{value!r}")
    if kind == "rate" and number > 100:
        raise ValueError(f"休學來源率超過100%：{field}={value!r}")
    return number
...
def fetch_source_frames(requests_module, pd_module, io_module, quote_module):
    ...
    for directory, path in current_paths:
        ...
        if frame is None:
            print(f"suspend source missing: static_file/{directory}")
            continue
        year_col = _find_first(frame.columns, "學年度")
        if year_col is None:
            raise ValueError(f"休學 CSV 缺少學年度欄：{path}")
    ...
    if legacy is None:
        raise RuntimeError(f"找不到休學歷史檔：{legacy_path}")
    year_col = _find_first(legacy.columns, "學年度")
    if year_col is None:
        raise ValueError("休學歷史 CSV 缺少學年度欄")
    ...
    if not selected:
        raise RuntimeError("沒有抓到任何休學學年度資料")
...
def _assert_reconciliation(values, roles, year, context, rec):
    def check(parts, total, label):
        ...
        if abs(sum(parts) - total) > 0.5:
            raise ValueError(
                f"{year} 學年度休學對帳失敗（{context}／{label}）："
                f"分項 {parts}、小計 {total}"
            )
        rec["checks_passed"] += 1
...
def _check_scale_anomalies(metrics):
    ...
    for year, value in usable.items():
        ...
        if anomalous:
            raise ValueError(
                f"休學年度尺度異常：{year}={value}，其餘年度中位數={baseline}；"
                "若確認為來源範圍異常，請明確加入 KNOWN_BAD_YEARS"
            )
    print(f"suspend scale metrics =========== {metrics}")
...
def transform_frames(source_frames, lookup, data_time, pd_module, json_module):
    ...
    if data.empty:
        raise RuntimeError("休學資料轉換後沒有新北市資料列")
...
dag = CommonDag(
    ...
    dag_folder="youth_edu_suspend_ntpc",
)
